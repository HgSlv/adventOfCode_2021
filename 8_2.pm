...
for (my $k = 0; $k < $CountBoard; $k++) {
    ...
    $hDictionary = getDictionary($Board[$k]);
    push(@Numbers, getNumber($Board[$k], $hDictionary));
}
...
sub getNumber {
    my ($BoardLine, $hWordDictionary) = @_;
    
    my @SplitPipe = split(/ \| /, $BoardLine);
    my @SplitWords = split(/ /, $SplitPipe[1]);

    my $Number = '';
    
    for (my $j = 0; $j < 4; $j++) {
        
        my $Word = join "", sort split //, $SplitWords[$j];
        
        $Number .= $hWordDictionary->{$Word};
    }
...
sub getDictionary {
    ...
    my $hWordDictionary = getWordDictionary($BoardLine);
    ...
    my $FoundWord = getNotInWord($BoardLine, $hWordDictionary, $hNumberDictionary->{1}, 6);
    $hNumberDictionary->{6} = $FoundWord;
    $hWordDictionary->{$FoundWord} = 6;

    $FoundWord = getNotInWord($BoardLine, $hWordDictionary, $hNumberDictionary->{4}, 6);
    $hNumberDictionary->{0} = $FoundWord;
    $hWordDictionary->{$FoundWord} = 0;

    $FoundWord = getNotInWord($BoardLine, $hWordDictionary, $hNumberDictionary->{0}, 6);
    $hNumberDictionary->{9} = $FoundWord;
    $hWordDictionary->{$FoundWord} = 9;

    $FoundWord = getInWord($BoardLine, $hWordDictionary, $hNumberDictionary->{7}, 5);
    $hNumberDictionary->{3} = $FoundWord;
    $hWordDictionary->{$FoundWord} = 3;

    $FoundCharC = getCharC($BoardLine, $hWordDictionary, $hNumberDictionary);
    $FoundWord = getInWord($BoardLine, $hWordDictionary, $FoundCharC, 5);
    $hNumberDictionary->{2} = $FoundWord;
    $hWordDictionary->{$FoundWord} = 2;

    my $FoundWord = getNotInWord($BoardLine, $hWordDictionary, $hNumberDictionary->{1}, 5);
    $hNumberDictionary->{5} = $FoundWord;
    $hWordDictionary->{$FoundWord} = 5;
    
    return $hWordDictionary;
}


sub getCharC {
    my ($BoardLine, $hWordDictionary, $hNumberDictionary) = @_;
    
    my @SplitWordToFind = split(//, $hNumberDictionary->{1});
    
    
    for (my $k = 0; $k < scalar @SplitWordToFind; $k++) {

        if (index($hNumberDictionary->{6}, $SplitWordToFind[$k]) == -1) {
            return $SplitWordToFind[$k];
        }
    }
...
sub getInWord {
    my ($BoardLine, $hWordDictionary, $WordToFind, $Length) = @_;
    ...
    for (my $j = 0; $j < 10; $j++) {
        ...
        if (!defined($hWordDictionary->{$SplitWords[$j]}) && length($SplitWords[$j]) == $Length) {
            ...
            for (my $k = 0; $k < scalar @SplitWordToFind; $k++) {
                ...
                if (index($SplitWords[$j], $SplitWordToFind[$k]) == -1) {
                    ...
                    #print ("\r $SplitWordToFind[$k] not found on $SplitWords[$j]\r");
                    $NotFound = 1;
                }
            }
