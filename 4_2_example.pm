...
for (my $i = 0; $i < $CountNumbers; $i++ ) {
    ...
    my @Boards = markNumberOnBoards(\@Boards, $CurrentNumber, $hWonCards);

    $hLineBingo = checkLineBingo(\@Boards);

    if ($hLineBingo->{'hasBingo'}) {
        
        unless (defined $hWonCards->{$hLineBingo->{'cardBingo'}}) {
            print ("\r Line Bingo found on Card: " . $hLineBingo->{'cardBingo'} . "\r");
            print ("\r Line Bingo found on Line: " . $hLineBingo->{'lineBingo'} . "\r");
            
            #$hLineBingo->{'won'} = $i;
            $hLineBingo->{'number'} = $CurrentNumber;
            $hWonCards->{$hLineBingo->{'cardBingo'}} = $hLineBingo;
        }
        #last;
    }
    ...
    if ($hColumnBingo->{'hasBingo'}) {
        unless (defined $hWonCards->{$hColumnBingo->{'cardBingo'}}) {
            print ("\r Column Bingo found on Card: " . $hColumnBingo->{'cardBingo'} . "\r");
            
            #$hColumnBingo->{'won'} = $i;
            $hColumnBingo->{'number'} = $CurrentNumber;
            $hWonCards->{$hColumnBingo->{'cardBingo'}} = $hColumnBingo;
...
print ("\r =================Result SUM: 13 =================== \r");
print (getBoardSum(\@Boards, 1) * 13);
print ("\r");


use Data::Dumper;
print ("\r =================hWonCards=================== \r");
print Dumper $hWonCards;
print ("\r");
...
sub markNumberOnBoards {
    ...
    for (my $i = 0; $i < $CountRows; $i++ ) {
        ...
        $CurrentCard = floor($i / 5);
        #print ("\r Line: $i => CurrentCard: $CurrentCard \r");
        
        if (defined($hWonCards->{$CurrentCard})) {
            #print ("\r Skipped:  $CurrentCard \r");
            next;
        }
        
        for (my $j = 0; $j < 5; $j++ ) {
            if ($Boards[$i][$j] == $Number) {
                #print ("\r Sractching Card:  $CurrentCard with number $Number\r");
                $Boards[$i][$j] = 888;
            }
        }
    }
